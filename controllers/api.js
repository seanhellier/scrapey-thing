...
router.get('/scrape', (req, res) => {
	...
	request('http://quotes.toscrape.com/', (error, response, body) => {
		if (!error && response.statusCode === 200) {
			...
			$('.quote').each(function(i, element) {
				...
				result.text = $(element).children('.text').text().trim();
				result.author = $(element).children('span').children('.author').text().trim();
				console.log('small author: ', result.author);
